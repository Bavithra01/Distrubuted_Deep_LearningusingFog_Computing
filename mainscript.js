document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            try {
                const data = JSON.parse(rawText); // Parse the JSON

                if (data.error) {
                    result.innerHTML = `<p class="error">Error: ${data.error}</p>`;
                } else {
                    result.innerHTML = `
		        <center><h1>Worker IP:</h1> ${data.workerIP || 'N/A'}</center>
                ...
                result.innerHTML = `<p class="error">Unexpected response format: ${rawText}</p>`;
            }
